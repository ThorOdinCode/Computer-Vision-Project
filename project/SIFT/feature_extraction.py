...
DEFAULT_OUTPUT_DIR = (
    PROJECT_DIR / "SIFT" / "extraction_samples"
)
...
def create_sift(
    ...
    if hasattr(cv2, "xfeatures2d") and hasattr(
        cv2.xfeatures2d,
        "SIFT_create"
    ):
...
def visualize_keypoints(
    ...
    """
    Draw the strongest SIFT keypoints.
...
def build_montage(
    ...
    cv2.putText(
        canvas,
        "Top SIFT keypoints shown in green",
        (margin, 62),
        cv2.FONT_HERSHEY_SIMPLEX,
        0.52,
        (210, 210, 210),
...
def process_sample_sequence(
    ...
    for image_number in DEFAULT_SAMPLE_IMAGE_IDS:
        ...
        output_path = (
            output_dir
            / f"{sequence}_{image_number}_sift.png"
        )
...
def process_dataset(
    ...
    print("=" * 72)
    print("HPatches SIFT Feature Extraction")
    print("=" * 72)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Run SIFT on the full HPatches sequence "
